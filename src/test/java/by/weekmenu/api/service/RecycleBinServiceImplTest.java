...
public class RecycleBinServiceImplTest {
    ...
    @Test
    public void restoreElement_Menu() {
        RecycleBin recycleBin = createRecycleBin("Вкусное меню", EntityNamesConsts.MENU, LocalDateTime.now());
        when(recycleBinRepository.findById(anyLong())).thenReturn(Optional.of(recycleBin));
        Menu menu = mock(Menu.class);
        when(menuRepository.findByNameIgnoreCase(anyString())).thenReturn(Optional.of(menu));
        recycleBinService.restoreElement(1L);
        verify(menuRepository, times(1)).restore(menu.getId());
    }
    ...
    @Test
    public void deleteElement_Menu() {
        RecycleBin recycleBin = createRecycleBin("Вкусное меню", EntityNamesConsts.MENU, LocalDateTime.now());
        when(recycleBinRepository.findById(anyLong())).thenReturn(Optional.of(recycleBin));
        Menu menu = mock(Menu.class);
        when(menuRepository.findByNameIgnoreCase(anyString())).thenReturn(Optional.of(menu));
